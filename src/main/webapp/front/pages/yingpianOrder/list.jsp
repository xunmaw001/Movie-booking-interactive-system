...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的影片订单 </p>
    </div>
    <!-- 标题 -->
                ...
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td style="width: 60px;">{{item.yingpianName}}</td>
                            <td style="width: 80px;">{{item.yingpianValue}}</td>
                            <td style="width: 250px;">
                                <img :src="item.yingpianPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
                            <td style="width: 60px;">{{item.piaofangNumber}}</td>
                            <td style="width: 100px;color: red">{{item.yingpianNewMoney}}</td>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 yingpianOrderContent 字段去除无用字符 如果字段长度大于60  就截取到60
            yingpianOrderContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            //列表查询
            ,pageList(yingpianOrderTypes) {
                this.yingpianOrderTypes = yingpianOrderTypes;
                // 获取列表数据
                layui.http.request('yingpianOrder/list', 'get', {
                    page: 1,
                    limit: this.limit,
                    yingpianOrderTypes: vue.yingpianOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('yingpianOrder/list', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    yingpianOrderTypes: this.yingpianOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                // 获取商品详情信息
                layui.http.requestJson(`yingpianOrder/pay`, 'PPST',{
                    id:id
                }, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
            ...
            ,refund(id) {
                // 获取商品详情信息
                layui.http.request(`yingpianOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        // 获取列表数据
        http.request('yingpianOrder/list', 'get',{
            page: 1,
            limit: limit,
            yingpianOrderTypes: vue.yingpianOrderTypes
        }, function(res) {
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('yingpianOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            yingpianOrderType: vue.yingpianOrderType
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }

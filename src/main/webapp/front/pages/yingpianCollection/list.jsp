            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">影片收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="yingpianName" id="yingpianName" placeholder="影片名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="yingpianTypes" id="yingpianTypes" lay-filter="yingpianTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in yingpianTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="piaofangNumberStart" id="piaofangNumberStart"  lay-skin="integer"   placeholder="票房开始" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <div class="layui-input-inline" style="margin-top: -5px">
                                  <input type="text" name="piaofangNumberEnd"   id="piaofangNumberEnd"    lay-skin="integer|piaofangNumberEnd"   placeholder="票房结束" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../yingpian/detail.jsp?id='+item.yingpianId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.yingpianPhoto?item.yingpianPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../yingpian/detail.jsp?id='+item.yingpianId)"><span>{{item.yingpianName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.yingpianNewMoney"><span style="color: red;">{{item.yingpianNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteYingpianCollection(item.id)">删除 {{item.yingpianName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 yingpianCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            yingpianCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteYingpianCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('yingpianCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        form.verify({
            ...
            ,piaofangNumberEnd: function(value, item){ //value：表单的值、item：表单的DOM对象
                var piaofangNumberStart = $("#piaofangNumberStart").val();
                if(piaofangNumberStart != null && piaofangNumberStart != "" && piaofangNumberStart != "null"){
                    var number = parseInt(value)- parseInt(piaofangNumberStart);
                    if(number<0){
                        return '票房的结束值不能小于开始值';
                    }
                }
            }
        ...
        function pageList() {
            ...
            if (jquery('#yingpianName').val() != null && jquery('#yingpianName').val() != "" && jquery('#yingpianName').val() != "null" ) {
                param['yingpianName'] = jquery('#yingpianName').val();
            }
                     
            var yingpianTypes = $("#yingpianTypes").find("option:selected").val();
            if (yingpianTypes != null && yingpianTypes != "" && yingpianTypes != "null") {
                            param['yingpianTypes'] = yingpianTypes;
                        }
                                         
            if (jquery('#piaofangNumberStart').val() != null && jquery('#piaofangNumberStart').val() != "" && jquery('#piaofangNumberStart').val() != "null" ) {
                param['piaofangNumberStart'] = jquery('#piaofangNumberStart').val();
            }
            if (jquery('#piaofangNumberEnd').val() != null && jquery('#piaofangNumberEnd').val() != "" && jquery('#piaofangNumberEnd').val() != "null" ) {
                param['piaofangNumberEnd'] = jquery('#piaofangNumberEnd').val();
            }
                                                            
            // 当前表
                                                    // 获取列表数据
            http.request('yingpianCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function yingpianTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=yingpian_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.yingpianTypesList = res.data.list;
               }
           });
       }
